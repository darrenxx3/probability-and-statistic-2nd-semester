...
```{r, message=FALSE}
#base R
judul <- "Distribusi Frekuensi Lokasi oleh Christopher Darren 00000054804" 
sbX <- "Lokasi"
sbY <- "Frekuensi"
legenda <- c("1 = metro, 2 = nonMetro")
barplot(frekuensi, main = judul, names.arg = legenda, xlab = sbX, ylab = sbY)
```
...
```{r, message=FALSE}
#base R
judul <- "Distribusi Frekuensi Lokasi vs Region oleh Christopher Darren 00000054804" 
sbX <- "Lokasi"
sbY <- "Frekuensi"
warna <- c("red", "blue")

legenda <- c("1 = metro", "2 = nonMetro")
barplot( frekuensi, main = judul, xlab = sbX, ylab = sbY, col = warna)
...
legenda <- c("1 = metro", "2 = nonMetro")
barplot(frekuensi, beside = TRUE, main = judul, xlab = sbX, ylab = sbY, legend = legenda, col = warna)
```
...
qcc::pareto.chart(frekuensi)

(frekuensi <-table(food$Region))
pareto.chart(frekuensi, col = warna)
warna <- c("red")

#Paling sering muncul: region NE
#Paling jarang muncul: region S
```
...
```{r}
judul <- "Histogram Annual Food Spending oleh Christopher Darren 00000054804"
hist(food$AnnualFoodSpending , xlim = c(0,20000),main = judul)
...
```{r}
plot(density(food$NetIncome), main = "Densitas Net Income oleh Christopher Darren 00000054804")
...
plot(food$NetIncome, food$AnnualFoodSpending, main = judul)
```
...
```{r}
  plot(food$NetIncome, food$AnnualFoodSpending, col = c("red", "blue"), pch = c(1,2), main = "Scatter Net Income vs plot Annual Food Spending oleh Christopher Darren 00000054804")
  legend("topleft",c("1","2"), pch = c(1,2), col = c("red","blue"))
```
...
```{r, message=FALSE}
#base R
boxplot(food$AnnualFoodSpending ~ food$Location , main = "Distribusi Spending location oleh Christopher Darren" )


#Median Spending lokasi 1 (Metro) ... spending lokasi 2
#Range Spending lokasi 1 lebih tinggi daripada range spending lokasi 2
```
...
```{r, message=FALSE}
#base R
boxplot(food$AnnualFoodSpending ~ food$Region , main = "Distribusi Food Spending bdskan Region oleh Christopher Darren" )
par(mfrow  = c(3,1))
hist(food$AnnualFoodSpending [food$Region== 4])
hist(food$AnnualFoodSpending[food$Region == 6])
hist(food$AnnualFoodSpending[food$Region == 8])

median(food$Region)
par(mfrow = c(1,1))
#Median Spending terendah pada region S
#Median Spending tertinggi pada region W
#Range Spending tertinggi pada region NE
#Region yang memiliki outlier: region NW
```
